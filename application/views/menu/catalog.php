        ...
        <div class="cat_container">
            ...
            foreach ($vars['catalog'] as $product){
                ...
                if($product['is_in_cart'] === true or $product['is_in_cart_database'] === true){
                    echo'<div class="product_buy_button"><a title="В Корзине" class="buy_button_catalog" category="'.$product['product_category_id'].'" id="'.$product['id_product'].'"><span class="info">В корзине</span><img class="buy_cart" src="/public/images/cart/cart_checked.png"></a></div>';
                }
                else{
                    echo'<div class="product_buy_button"><a title="Добавить в корзину" class="buy_button_catalog" category="'.$product['product_category_id'].'" id="'.$product['id_product'].'"><span class="info">Купить</span><img class="buy_cart" src="/public/images/cart/cart.png"></a></div>';
                }
                echo'</div>';
    ...
    <form  id="qqq">
        <ul  id="left-categories">
            ...
            foreach ($arr as $category => $hars) {
                ...
                echo '<li class="slide-li-back-fixed"><div id="'.transliterate($category).'" style="display: inline-block"><a  class="cat_image" id="'.transliterate($category).'" href="#category='.translit($category).'">' . $category . '</div><img alt="" class="img-cat" src="/public/images/PinClipart.com_salamander-clip-art_2150988.png">';
                echo '</a  >';
                echo '<ul style="display: flex; margin: 0; margin-top: 2px;" class="price_ul">
                ...
                if (is_array($hars) || is_object($hars)) {
                    foreach ($hars as $har => $chars) {
                        echo '<ul  style="display: block;" > ' . $har ;
